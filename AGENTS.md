...
- **Dual status item display**: Expanded status item selection to up to two ordered metrics from the same device. The Devices outline uses constrained checkboxes, and `StatusItemDisplayView` renders two selected values inside the single `NSStatusItem` as one vertical label (`AIR` or `RAD`) with two stacked value rows
